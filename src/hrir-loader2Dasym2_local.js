...
export default class HRIRloader2Dasym2_local {
    ...
    getHoaFilterFromHrirFilterL(nCh, nSamples, sampleRate, hrirs, decodingMatrix) {
        ...
        let hoaBufferL = this.context.createBuffer(nCh, nSamples, sampleRate);
        // END of EDIT
    ...
    getHoaFilterFromHrirFilterR(nCh, nSamples, sampleRate, hrirs, decodingMatrix) {
        // create empty buffer ready to receive hoa filters
        if (nSamples>hrirs[0][0].length) nSamples = hrirs[0][0].length;
        let hoaBufferR = this.context.createBuffer(nCh, nSamples, sampleRate);
        
        // sum weighted HRIR over Ambisonic channels to create HOA IRs
        for (let i = 0; i < nCh; i++) {
            let concatBufferArrayRight = new Float32Array(nSamples);
            for (let j = 0; j < hrirs.length; j++) {
                for (let k = 0; k < nSamples; k++) {
